...
% ============================================================
%  Appendix B — Registered Hypotheses Data (Anonymized)
...
period. Registration timestamps span 11~February~2026 through 12~April~2026 (61~calendar days, approximately nine weeks), covering five consecutive two-week sprint periods from the earliest to the latest creation date in the organization's cadence. All entity references, service names, and internal artifact links have
been anonymized in accordance with the ethical agreement described in
Section~\ref{sec:ethical-considerations}. The anonymization legend is provided
in Table~\ref{tab:entity-legend}. Technical planning entries are reproduced in
full in Section~\ref{sec:app-technical-plans}.

% ─────────────────────────────────────────────────────────────
\section{Anonymization Legend}
...
under the ethical agreement summarized in Section~\ref{sec:ethical-considerations},
this appendix does not reproduce real catalog identifiers. Instead, each hypothesis
table in Section~\ref{sec:app-hypothesis-records} lists \emph{anonymized} entity
references in its \textbf{Entities} row; Table~\ref{tab:entity-legend} explains what
each anonymized string denotes at a functional level, without recovering the original
...
legend immediately available when reading the hypothesis records, rather than
deferring it past the large tables in Section~\ref{sec:app-hypothesis-records}.
...
\toprule
\textbf{Anonymized entity reference} & \textbf{Description} \\
\midrule
...
\texttt{platform: shared streaming infrastructure}
  & Shared Kafka streaming platform and consumption standards, modeled as a
    catalog component. \\
\bottomrule
\end{tabular}
\captionof{table}{Anonymized Backstage entity references and their descriptions.}
\label{tab:entity-legend}
\end{minipage}
\par\bigskip

% ─────────────────────────────────────────────────────────────
\section{Hypothesis Records}
\label{sec:app-hypothesis-records}
% ─────────────────────────────────────────────────────────────

Each hypothesis is typeset as a single record table: fixed metadata rows, a
full-width statement row, then author notes and technical-plan references (see
Section~\ref{sec:app-technical-plans}).
The bold header on each record repeats the hypothesis identifier and short label
exactly as in Table~\ref{tab:hypotheses-overview} of Chapter~\ref{chap:results}.
...
\hypoboxtail%
  {Validated through a technical discovery spike that defined the transfer
   capability and produced a prioritized implementation backlog.}%
...
description of the planned action, and the expected outcome. Internal artifact
...
\textit{Action type:} Modularization / Flexibility \quad \textit{Target date:} 2026-03-31 \\
...
%% ─── H05 plan ─────────────────────────────────────────────
\paragraph{TP-05-A \textnormal{(H05 — Credit-bureau context-agnostic restructuring)}}
...
\textit{Expected outcome:} A prioritized plan for context-agnostic restructuring and a
...
%% ─── H07 plan ─────────────────────────────────────────────
\paragraph{TP-07-A \textnormal{(H07 — Blob ownership transfer across service domains)}}
...
%% ─── H08 plan ─────────────────────────────────────────────
\paragraph{TP-08-A \textnormal{(H08 — Blob cross-client sharing governance)}}
...
\textit{Expected outcome:} A governance decision enabling prioritization of
sharing and transfer implementation work.

%% ─── H09 plans ────────────────────────────────────────────
\paragraph{TP-09-A \textnormal{(H09 — DLT for audit vs.\ offset-based replay)}}
