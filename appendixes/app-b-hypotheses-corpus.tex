...
% ============================================================
%  Appendix B — Registered Hypotheses Corpus (Anonymised)
...
\begin{table}[htbp]
\caption{Anonymised Backstage entity references and their descriptions.}
...
\texttt{internal-framework: JVM platform stack with Kafka integration}
  & The organisation's shared application framework for JVM services, covering
...
Each hypothesis profile below follows a standard layout: identifier, originating
team, registration date, lifecycle status, source type (Requirements or Solution),
affected Backstage entity references, quality attributes, uncertainty level,
impact level, the hypothesis statement, and contextual notes provided by the
author. Technical plans are cross-referenced to Section~\ref{sec:app-technical-plans}.

\newcommand{\hypobox}[9]{%
  % #1 ID label  #2 Team  #3 Date  #4 Status  #5 Source
  % #6 Entities  #7 QA    #8 U/I   #9 Statement
  \noindent\begin{minipage}{\linewidth}
  \vspace{6pt}
  \noindent\rule{\linewidth}{0.4pt}
  \smallskip
  \noindent\textbf{#1}\hfill\textit{Team: #2 \textbar{} Registered: #3 \textbar{} Status: #4}\\[2pt]
  \noindent\textit{Source:} #5 \quad \textit{Uncertainty / Impact:} #8 \\
  \noindent\textit{Quality attributes:} #7 \\
  \noindent\textit{Entities:} \texttt{#6} \\[4pt]
  \noindent\textit{Statement:} ``#9''
  \vspace{4pt}
  \end{minipage}%
}
...
\noindent\textit{Notes:} The organisation intends to deprecate the legacy stack
in that domain; the Kotlin + JOOQ + Spring template is the leading candidate to
replace it.

\noindent\textit{Technical plans:} None registered at end of observation period.
...
\noindent\textit{Notes:} Subject on hold; technical plans expected for the
following quarter.
...
\noindent\textit{Notes:} The platform framework must let applications derive
full log context including database logs. It is expected to supersede a legacy
audit library and satisfy organisation-wide security audit requirements.

\noindent\textit{Technical plans:} TP-03-A, TP-03-B, TP-03-C
(see Section~\ref{sec:app-technical-plans}).
...
\hypobox%
  ...
  {Medium / Very Low}%
  {The secure blob storage service must retain a reference to the data subject
   in its domain data so all sensitive blobs can be deleted when the user
   exercises their right to erasure under applicable privacy regulation.}

\noindent\textit{Notes:} Concern raised in an internal RFC about fully erasing
user sensitive data, including binary objects.
...
\noindent\textit{Notes:} Discovery focused on: (1)~dependency matrix —
current vs.\ real dependency per integration; (2)~agnostic interface — contract
without domain concepts, only minimal identifiers and bureau outputs;
(3)~migration plan — which flows can isolate immediately vs.\ requiring deeper
business-logic refactors.

\noindent\textit{Technical plans:} TP-05-A
(see Section~\ref{sec:app-technical-plans}).
...
\hypobox%
  ...
  {Chunked keyset pagination with isolated repository-level transactions keeps
   memory strictly bounded by chunk size and prevents out-of-memory failures
   when aggregating millions of billing records.}

\noindent\textit{Notes:} Validated with a capacity/volume test at 5$\times$
production scale (45M rows total, 1.5M rows/month). Profiling showed JPA streams
can cause first-level cache growth; chunking scopes transactions ($\approx$10k rows)
so the garbage collector can reclaim memory (sawtooth heap pattern). Composite
index on \texttt{(billing\_year\_month, status, id)}.

\noindent\textit{Technical plans:} TP-06-A
(see Section~\ref{sec:app-technical-plans}).
...
\noindent\textit{Notes:} Validated through a technical discovery spike that
defined the transfer capability and produced a prioritised implementation backlog.

\noindent\textit{Technical plans:} TP-07-A
(see Section~\ref{sec:app-technical-plans}).
...
\hypobox%
  ...
  {Very Low / Very High}%
  {Secure blob storage must support sharing a blob across multiple clients so
   the same object can be read from different domain services.}

\noindent\textit{Notes:} Example: a customer-facing channel receives documents
later routed to support tooling; support needs read access in chat history
without breaking vault invariants.

\noindent\textit{Technical plans:} TP-08-A
(see Section~\ref{sec:app-technical-plans}).
...
\noindent\textit{Notes:} How to falsify: controlled replay from the DLT without
mutating the event must be proven safer, simpler, and more auditable than replay
by offset.

\noindent\textit{Technical plans:} TP-09-A, TP-09-B
(see Section~\ref{sec:app-technical-plans}).
...
\hypobox%
  ...
  {For short-lived transient failures, limited synchronous retry on the consumer
   thread preserves per-partition ordering better and with lower overall
   complexity than asynchronous retry on a separate queue.}

\noindent\textit{Notes:} How to falsify: a per-partition retry queue preserves
order with less blocking time and acceptable operational cost.

\noindent\textit{Technical plans:} TP-10-A
(see Section~\ref{sec:app-technical-plans}).
...
\noindent\textit{Notes:} How to falsify: modelling failure state in Kafka shows
lower operational complexity and equal or better recovery than handling failures
in the execution layer.

\noindent\textit{Technical plans:} TP-11-A
(see Section~\ref{sec:app-technical-plans}).
...
\hypobox%
  ...
  {Medium / High}%
  {Classifying errors as recoverable vs.\ non-recoverable before applying
   retry reduces consumer lag and computational cost without worsening the
   successful processing rate.}

\noindent\textit{Notes:} How to falsify: indiscriminate retries improve final
success rate without meaningful latency/capacity cost.

\noindent\textit{Technical plans:} TP-12-A, TP-12-B
(see Section~\ref{sec:app-technical-plans}).
...
\noindent\textit{Notes:} How to falsify: a formal correction mechanism fully
preserves audit trail, causality, and reproducibility of the event.

\noindent\textit{Technical plans:} TP-13-A
(see Section~\ref{sec:app-technical-plans}).
