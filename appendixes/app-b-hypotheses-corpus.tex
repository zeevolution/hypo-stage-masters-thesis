...
Hypotheses in Hypo-Stage are linked to components in the organisation's Backstage
...
profile in Section~\ref{sec:app-hypothesis-profiles} lists \emph{anonymised} entity
...
legend immediately available when reading the hypothesis profiles, rather than
deferring it past the large tables in Section~\ref{sec:app-hypothesis-profiles}.
...
Each hypothesis profile is typeset as a single table: fixed metadata rows, a
...
\hypoboxnine%
  ...
  {The Kotlin + JOOQ + Spring service template is ready to scale for use in
   ...
   stack.}%
\hypoboxtail%
  {The organisation intends to deprecate the legacy stack in that domain; the
...
\hypoboxtail%
  {The platform framework must let applications derive full log context including
   ...
   organisation-wide security audit requirements.}%
