...
This preamble makes the correspondence explicit so that readers can assess
compliance at a glance.

\paragraph{Data collection.}
Three data sources were employed, as specified in Section~\ref{sec:data-collection}:
\begin{enumerate}
  ...
  \item \textbf{Artifact analysis.} At the end of the observation period, the
    complete set of hypotheses, assessments, technical plans, and lifecycle
    records created by each team in Hypo-Stage was extracted and analyzed
    ...
    are derived exclusively from this artifact record.
  \item \textbf{Open-ended active listening sessions.} The researcher
    ...
    (Section~\ref{sec:listening-guide}). Verbatim statements from these
    sessions are reported in this chapter only when they add evidence beyond
    what is already documented in field notes or artifacts; in every case the
    participant's role is identified (e.g., ``Product Team technical leader'').
\end{enumerate}

\paragraph{Data analysis — three-stage thematic coding.}
All field notes, artifact summaries, and active listening records were analyzed
using the thematic coding procedure described by \citet{robson2016} for
qualitative data in flexible designs (Section~\ref{sec:data-analysis}). The three stages
prescribed in the research design were followed as follows:
\begin{enumerate}
  \item \textbf{Stage 1 — Inductive open coding.} All primary data were
    ...
    capturing recurring observations and verbatim themes without imposing
    prior categories. Examples of codes generated at this stage include:
  ...
  \item \textbf{Stage 2 — Axial grouping into conceptual categories.} Codes
    ...
    of the conceptual framework (Figure~\ref{fig:conceptual-framework}): patterns of tool use ($\rightarrow$ RQ1,
    Section~\ref{sec:results-rq1}); types of architectural uncertainty and structural archetypes
    ($\rightarrow$ RQ2, Sections~\ref{sec:rq2-results} and~\ref{sec:group-b-patterns}); perceived benefits ($\rightarrow$ RQ3,
    Section~\ref{subsec:benefits}); perceived challenges ($\rightarrow$ RQ3, Section~\ref{subsec:challenges}); candidate
    tool refinements ($\rightarrow$ RQ4, Section~\ref{sec:refinements}); and organizational conditions for
    adoption ($\rightarrow$ Group~A patterns, Section~\ref{sec:group-a-patterns}).
...
and illustrate the qualitative findings. Following \citet{robson2016}, the
...
\begin{figure}[ht]
  \centering
  \placeholder[0.85\textwidth]{Hypo-Stage EntityHypothesesTab --- the plugin tab
    rendered inside a Backstage component page, showing only the hypotheses
    linked to that specific catalog entity. The tab header, hypothesis list,
    and status indicators are visible in the standard Backstage component layout.}
...
\begin{figure}[ht]
  \centering
  \placeholder[0.85\textwidth]{Hypo-Stage Prioritization Dashboard --- the four-quadrant
    focus panel showing hypotheses sorted by urgency. Quadrants: Needs Attention
    (high U + high I), Can Postpone (low U + low/high I), Monitor (medium U + medium I),
    and Resolved. Each hypothesis card shows ID, short label, and ratings.}
  \caption{Prioritization Dashboard in Hypo-Stage: the focus panel surfaces hypotheses
    ...
    operationalizing the Postpone Uncertain Decisions pattern from \citet{silva2024patterns}.}
...
\begin{figure}[ht]
  \centering
  \placeholder[0.8\textwidth]{Hypo-Stage Evolution Chart --- showing the shift in
    uncertainty and impact ratings across successive assessments for a single
    hypothesis. X-axis: assessment date; Y-axis: rating (Very Low to Very High);
    two lines: Uncertainty (solid) and Impact (dashed).}
...
\begin{figure}[ht]
  \centering
  \placeholder[0.75\textwidth]{Hypo-Stage hypothesis creation form --- showing the
    Statement field with inline contextual guidance text (e.g.\ ``Start from a
    belief: We believe X is true. Then state what observable outcome would prove
    it false.'') and the multi-select Quality Attributes field.}
...
within a single organizational context over a four-week observation period.
This dataset is sufficient to motivate conceptual characterization and to propose
initial vocabulary for hypothesis engineers, but it does not support statistical
...
teams but has not been tested in the broader population of software engineering
organizations. They are presented here as candidate patterns, explicitly
requiring further empirical validation before adoption as guidelines.
...
\emph{Statistical scope:} All five archetypes were identified from the same
thirteen-hypothesis dataset. While each archetype maps to two or more
hypotheses in the dataset (and in some cases corresponds to a single rich
instance, as discussed in the individual pattern entries), the small dataset
size means that no frequency claims are made. The contribution lies in naming
and structuring recurring shapes so that future practitioners and researchers
have a vocabulary for recognition --- not in establishing that any archetype
will appear with a particular frequency in other organizational contexts.
Consolidating these patterns into the broader ArchHypo pattern language is
identified as a priority future-work item in Section~\ref{sec:future-work}.
