...
\subsection*{What this section is for}

This section adds \emph{no new findings}. Its role is practical: it tells readers,
supervisors, and examiners \textbf{where in Chapter~\ref{chap:results} each research
question is answered}, so that review of the dissertation can move quickly between
the question statements in Chapter~\ref{chap:introduction} and the corresponding
body of evidence. Use the roadmap below as a cross-index; use the short synthesis
that follows for a condensed statement of how the evidence hangs together.

\subsection*{Roadmap: research question $\rightarrow$ evidence in this chapter}

\noindent\textbf{RQ1 (tool use).}\ 
Primary material: Section~\ref{sec:rq1-results}.\
Synthetic reading (with RQ3): Group~A patterns in Section~\ref{sec:group-a-patterns}.

\medskip
\noindent\textbf{RQ2 (uncertainties and structural shapes).}\ 
Typological corpus account: Section~\ref{sec:rq2-results}.\
Structural archetypes (Group~B): Section~\ref{sec:group-b-patterns}.

\medskip
\noindent\textbf{RQ3 (benefits and challenges).}\ 
Primary material: Section~\ref{sec:rq3-results}.\
Synthetic reading (with RQ1): Group~A patterns in Section~\ref{sec:group-a-patterns}.

\medskip
\noindent\textbf{RQ4 (refinements).}\ 
Primary material: Section~\ref{sec:rq4-results} (nine categories, each observation-led).

\subsection*{Closing synthesis}

\textbf{RQ2.}
The wording of RQ2 deliberately couples a typological task with a structural one.
Section~\ref{sec:rq2-results} carries the former; Section~\ref{sec:group-b-patterns}
carries the latter. Read together, they show both \textit{what} was registered and
\textit{what shapes} recurred in the corpus---the combination that Chapter~\ref{chap:introduction}
encodes in a single question.

\textbf{RQ1 and RQ3.}
Group~A is the bridge construct: it summarises conditions for adoption that are
visible only when use (RQ1) and reported experience (RQ3) are read jointly. Neither
RQ1 nor~RQ3 needs rewording; the contribution is to make explicit what ``an answer''
must contain in this organisational setting.

\textbf{RQ4.}
Section~\ref{sec:rq4-results} answers RQ4 in the sense required by design-science
reporting: refinements appear as outcomes tied to observations, not as a detached
product log.
