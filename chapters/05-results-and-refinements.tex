...
\begin{enumerate}
  \item \textbf{Stage 1 — Inductive open coding:} All primary data were
    ...
    prior categories. Representative open codes, each tied explicitly to where
    the corresponding observations are developed later in this chapter, include:
    \textit{kafka-cluster-burst-registration}
    (Section~\ref{subsec:registration-dynamics}; hypothesis-cluster pattern~B1,
    Section~\ref{sec:group-b-patterns});
    \textit{senior-author-only}
    (Section~\ref{subsec:registration-dynamics}; Group~A pattern~A1,
    Section~\ref{sec:group-a-patterns}); and
    \textit{registration-barrier-time-pressure}
    (Section~\ref{subsec:registration-dynamics}; perceived challenges,
