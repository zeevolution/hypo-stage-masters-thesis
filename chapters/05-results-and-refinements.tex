...
teams used the tool and the technique in practice; RQ2 characterises the
...
This section closes the chapter by mapping the preceding evidence back to the
four research questions stated in Chapter~\ref{chap:introduction}. The mapping is
split between RQ2---which received an explicit extension through Group~B---and the
remaining questions, which are answered along the lines originally formulated.

\subsection{RQ2: typological coverage and structural patterns}

RQ2 asks: \textit{``What types and recurring structural shapes of architectural
uncertainties do these teams identify, record, and manage using Hypo-Stage, and
what patterns emerge from their content and organisational context?''}

The first clause is answered in Section~\ref{sec:rq2-results}. That section
catalogues the registered hypotheses by source, quality attribute, lifecycle,
and technical-plan behaviour. Together, these dimensions provide a typological
account of what the teams chose to make explicit in Hypo-Stage.

The second clause is answered in Section~\ref{sec:group-b-patterns}. The five
Group~B patterns name \textit{recurring structural shapes} in the corpus itself.
They therefore lift the analysis from instance-level description to a reusable
vocabulary for hypothesis engineers. Taken together, Sections~\ref{sec:rq2-results}
and~\ref{sec:group-b-patterns} realise the refined formulation of RQ2 at the level
of the research question rather than as an informal appendix to the typology.

\subsection{RQ1, RQ3, and RQ4}

\textbf{RQ1 and RQ3.}
Section~\ref{sec:group-a-patterns} distils four Group~A patterns from the
intersection of tool-use evidence (RQ1; Section~\ref{sec:rq1-results}) and
participant accounts of benefits and friction (RQ3;
Section~\ref{sec:rq3-results}). These patterns concern organisational conditions
for sustaining hypothesis engineering; they do not call for a change in how RQ1
or~RQ3 is worded, but they sharpen what an answer to those questions must cover.

\textbf{RQ4.}
Section~\ref{sec:rq4-results} documents nine refinement categories. Each category
is tied to a concrete observation and to the artefact changes that implement it.
That structure matches the design-science intent of~RQ4: refinements are reported
as empirical outcomes, not as a separate commentary.
