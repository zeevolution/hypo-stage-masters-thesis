...
Hypo-Stage that emerged from them. The results are organised around the four
research questions defined in Chapter~\ref{chap:introduction}: RQ1 addresses how
teams used the tool and the technique in practice; RQ3 characterises the
architectural uncertainties they registered; RQ4 reports the benefits and
challenges perceived by participants; and RQ2 documents each refinement, tracing
...
questions by drawing two families of \textit{emerging patterns}: team-behaviour
...
they used it to externalise concerns that had until then existed only as tacit
...
already the daily entry point for service catalogue, documentation, and CI/CD
...
% ─────────────────────────────────────────────────────────────
\section{RQ3 — Architectural Uncertainties Registered}
\label{sec:rq3-results}
\label{sec:results-rq3}
% ─────────────────────────────────────────────────────────────

\subsection{Corpus Overview}
\label{subsec:corpus-overview}

Table~\ref{tab:hypotheses-overview} presents the thirteen hypotheses registered
during the study, anonymised in accordance with the ethical agreement described
in Section~\ref{sec:ethical-considerations}. Each hypothesis is identified by a
...
Table~\ref{tab:qa-distribution} summarises the frequency of quality attributes
...
forces: the organisation's internal security audit requirements, which drove H03,
...
obligations crystallise into architectural hypotheses — a pattern discussed in
...
of acting incorrectly are severe. Hypo-Stage's prioritisation dashboard surfaced
...
the end of the observation period. Together these seven hypotheses carried twelve
individual technical planning entries. Table~\ref{tab:plan-strategies} summarises
the strategies used.

\begin{table}[htbp]
\caption{Technical plan strategies observed across the corpus.}
...
% ─────────────────────────────────────────────────────────────
\section{RQ4 — Perceived Benefits and Challenges}
...
\paragraph{Externalisation of tacit architectural knowledge.}
...
\paragraph{Prioritisation support.}
...
Both technical leaders expressed the view that realising the collaborative
potential of hypothesis engineering would require the team to institutionalise at
least one touchpoint where the backlog is reviewed together.

% ─────────────────────────────────────────────────────────────
\section{RQ2 — Refinements to Hypo-Stage}
...
\texttt{ceee509} (the artefact boundary established by \citealt{correa2025}) to
...
primarily addresses. All refinements address RQ2 by definition; selected
refinements also bear on RQ1 (adoption ease) and RQ4 (usability and learning
curve).
...
which degraded usability as the corpus grew.
...
\subsection{Prioritisation Dashboard}
...
with explanatory subtext aligned with ArchHypo's prioritisation model
...
\textbf{Impact (RQ4).} The "Can Postpone" segment was specifically cited by
...
exact hypothesis statement before proceeding (paste-enabled to avoid penalising
...
Table~\ref{tab:refinements-map} summarises the nine categories of refinements,
...
active listening transcripts, and the hypothesis artifact corpus produced two
families of patterns. Group~A describes team-behaviour conditions that shape
...
The four Group~A patterns emerge from triangulating RQ1 and RQ4 findings. They
describe organisational and individual-level conditions that the study observed
...
a common trait: they concerned uncertainties whose consequences would materialise
...
hypothesis backlog. Their architectural horizon and organisational authority
position them to recognise which uncertainties are worth formalising. Non-senior
...
day-to-day." The observed corpus of thirteen hypotheses is known to
...
from sprint ceremonies, where engineers externalise concerns that have been
...
a question that only becomes answerable through an empirical corpus of
...
source type, quality-attribute profile, and relation to its organisational
context — should an engineer recognise in order to write and manage it more
effectively?}
\end{quote}
...
\citep{silva2024patterns}. The thirteen-hypothesis corpus produced by this
...
of this work: they are grounded in a real hypothesis corpus, not derived
from theoretical analysis, and they provide a vocabulary that hypothesis
engineers can use both to recognise familiar situations more quickly and to
...
\textbf{Observed evidence.} Two hypotheses in the corpus arise directly from
regulatory obligations: H04 (the GDPR right-to-erasure requirement for binary
documents in the blob storage service) and H03 (organisation-wide security
...
implementation is complete, yet its characterisation as "Open" may suggest
...
capacity test artefact explicitly, preserving the evidentiary chain.
...
prioritisation of the transfer and sharing implementation work. Quality attributes
...
\paragraph{On RQ3 and the emerging patterns.}

The current formulation of RQ3 — \textit{"What types of architectural
uncertainties do these teams identify, record, and manage using Hypo-Stage over
the course of their work?"} — adequately covers the characterisation of
uncertainty by source, quality attribute, and lifecycle presented in
Section~\ref{sec:rq3-results}. However, the Group~B patterns in
Section~\ref{sec:group-b-patterns} extend the answer to RQ3 in a direction that
the original formulation did not anticipate: they characterise not merely
\textit{what} uncertainties were registered but the \textit{recurring structural
shapes} those uncertainties take and how those shapes carry implications for how
hypotheses should be written and managed. This extension represents a contribution
that goes beyond cataloguing observed instances.

Accordingly, the authors recommend that future replications of this study adopt
a refined version of RQ3: \textit{"What types and recurring structural shapes of
architectural uncertainties do these teams identify, record, and manage using
Hypo-Stage, and what patterns emerge from their content and organisational
context?"} This reformulation makes the Group~B pattern contribution explicit
at the research question level rather than treating it only as a supplementary
finding.

\paragraph{On RQ1, RQ2, and RQ4.}

The four Group~A patterns (Section~\ref{sec:group-a-patterns}) emerge from the
intersection of RQ1 and RQ4 findings: they describe the organisational conditions
under which hypothesis engineering adoption was observed to succeed or fail.
These patterns are consistent with the current formulations of RQ1 (tool use
in practice) and RQ4 (perceived benefits and challenges) and do not require
reformulation of those questions. RQ2 is fully addressed by the nine categories
of Hypo-Stage refinements documented in Section~\ref{sec:rq2-results}, each
traced to a specific observation.
